...
# PRE-ASSIGNED RECEPTACLE POSITION COORDINATES: (Base, Shoulder, Elbow)
SORTING_BIN_MAP = {
    "M6":  (-55, 10.0, -100.0),
    "M8":  (-45.0, 10, -80.0),
    "M10": (-35, 5.0, -30),
...
class VisualPathCompiler:
    ...
    def compile_simulated_sequence(self, sequences):
        for command, value in sequences:
            ...
            for i in range(1, 9): # 8 intermediate frame steps per motor update
...
def main():
    ...
    for result in results:
        for box in result.boxes:
            ...
            if angles is not None:
                ...
                print(f"[TARGET FOUND] Type: {size_str} -> Kinematics Pick: {[round(a,1) for a in angles]} | Bin Drop: {drop_bin_pose}")
                
                # Append task mapping tuple: (Pick Joint Set, Drop Joint Set)
                compiled_tasks.append((angles, drop_bin_pose))
                
                # Draw label and tracking visuals on screen
    ...
    ax1.set_title(f"Classified Scene Frame ({len(compiled_tasks)} screws)")
    ax1.axis('off')
    
    ax2 = fig.add_subplot(122, projection='3d')
    print(f"\n[SIMULATION] Rendering nested operations graph ({len(compiler.step_history)} frame movements)...")
    
    for step_joints in compiler.step_history:
        draw_3d_robot(ax2, step_joints, kinematics, "NESTED CYCLE RUNNING...")
        plt.pause(0.01)

    print("[SUCCESS] Full sorting cycle finished and verified successfully.")
    plt.ioff()
    plt.show()
